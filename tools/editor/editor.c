...
void render_grid(SDL_Renderer *renderer, int offset_x, int offset_y,
  ...
  if (start_x > 0)
    start_x -= tile_size_scaled;

  float start_y = -fmodf(offset_y, tile_size_scaled);
  if (start_y > 0)
    start_y -= tile_size_scaled;
...
void render_editor_win(Editor *e, float delta_time) {
  ...
  if (e->settings.render_layer_1) {
    render_layer(&e->tile_map->render_layers[0], e->e_render, 0, e->asset_map,
                 &e->e_zoom, delta_time);
  }

  if (e->settings.render_layer_2) {
    render_layer(&e->tile_map->render_layers[1], e->e_render, 0, e->asset_map,
                 &e->e_zoom, delta_time);
  }

  if (e->settings.render_layer_3) {
    render_layer(&e->tile_map->render_layers[2], e->e_render, 0, e->asset_map,
                 &e->e_zoom, delta_time);
  }

  render_grid(e->e_render, e->e_zoom.offset_x, e->e_zoom.offset_y,
              e->e_zoom.scale);
  ...
  if (e->select_buf.active_selection) {
    ...
    SDL_Rect dest = {snapX, snapY, e->select_buf.rect.w * e->e_zoom.scale,
                     (e->select_buf.rect.h * e->e_zoom.scale)};
    SDL_SetRenderDrawBlendMode(e->e_render, SDL_BLENDMODE_BLEND);
    if (e->settings.is_entity) {
      SDL_SetRenderDrawColor(e->e_render, 255, 0, 0, 100);
    } else {
...
void render_tilesheet_win(Editor *e) {
  ...
  SDL_Rect t_zoom = {e->t_zoom.offset_x, e->t_zoom.offset_y,
                     e->t_w * e->t_zoom.scale, e->t_h * e->t_zoom.scale};
  ...
  if (is_window_focused(e->t_win) && e->t_mouse.left_pressed == 1) {
    ...
    world_to_screen(e->t_zoom.offset_x, e->t_zoom.offset_y, e->t_zoom.scale,
                    e->t_mouse.x, e->t_mouse.y, &scaled_cur_x, &scaled_cur_y);
    world_to_screen(e->t_zoom.offset_x, e->t_zoom.offset_y, e->t_zoom.scale,
                    e->t_mouse.click_x, e->t_mouse.click_y, &scaled_select_x,
                    &scaled_select_y);
    ...
    if (e->settings.is_entity) {
      SDL_SetRenderDrawColor(e->t_render, 255, 0, 0, 100);
    } else {
  ...
  if (e->select_buf.active_selection) {
    ...
    world_to_screen(e->t_zoom.offset_x, e->t_zoom.offset_y, e->t_zoom.scale,
                    e->select_buf.rect.x, e->select_buf.rect.y, &scaled_x,
                    &scaled_y);

    SDL_Rect selection = {scaled_x, scaled_y,
  ...
  render_grid(e->t_render, e->t_zoom.offset_x, e->t_zoom.offset_y,
              e->t_zoom.scale);
...
void render_settings_win(Editor *e) {
  ...
  if (nk_begin(&e->s_nk_state->ctx, "Settings", nk_rect(0, 0, 300, 768),
    ...
    if (nk_option_label(&e->s_nk_state->ctx, "Layer 1", e->settings.layer == 0))
      e->settings.layer = 0;
    if (nk_option_label(&e->s_nk_state->ctx, "Layer 2", e->settings.layer == 1))
      e->settings.layer = 1;
    if (nk_option_label(&e->s_nk_state->ctx, "Layer 3", e->settings.layer == 2))
      e->settings.layer = 2;
    ...
    nk_layout_row_dynamic(&e->s_nk_state->ctx, 30, 1);
    nk_checkbox_label(&e->s_nk_state->ctx, "Collidable",
                      &e->settings.collidable);
    ...
    nk_layout_row_dynamic(&e->s_nk_state->ctx, 30, 3);
    nk_checkbox_label(&e->s_nk_state->ctx, "Layer 1",
                      &e->settings.render_layer_1);
    nk_checkbox_label(&e->s_nk_state->ctx, "Layer 2",
                      &e->settings.render_layer_2);
    nk_checkbox_label(&e->s_nk_state->ctx, "Layer 3",
                      &e->settings.render_layer_3);

    nk_spacer(&e->s_nk_state->ctx);

    nk_layout_row_dynamic(&e->s_nk_state->ctx, 30, 3);
    nk_checkbox_label(&e->s_nk_state->ctx, "Is Entity", &e->settings.is_entity);

    if (e->settings.is_entity) {
      nk_layout_row_dynamic(&e->s_nk_state->ctx, 30, 1);
      nk_property_int(&e->s_nk_state->ctx, "Frame Rows:", 0,
                      &e->settings.frame_rows, 10, 1, 1);
      nk_property_int(&e->s_nk_state->ctx, "Frame Columns:", 0,
                      &e->settings.frame_columns, 10, 1, 1);
      nk_property_float(&e->s_nk_state->ctx, "Frame Duration:", 0,
                        &e->settings.frame_duration, 10, 0.5, 1);
    }
  }
...
void update_editor_entities(Editor* editor, float delta_time) {
  for (int i = 0; i < 2; i++) {
    for (int j = 0; j < editor->tile_map->render_layers[i].ent_count; j++) {
      update_entity(&editor->tile_map->render_layers[i].ents[j], delta_time);
    }
  }
}
...
static void init_windows(Editor *e) {
  ...
  e->e_win = SDL_CreateWindow("Tilemap Editor", SDL_WINDOWPOS_CENTERED,
  ...
  e->t_win =
      SDL_CreateWindow("Tilesheets", SDL_WINDOWPOS_UNDEFINED,
                       SDL_WINDOWPOS_UNDEFINED, 500, 900, SDL_WINDOW_SHOWN);
  SDL_SetWindowPosition(e->t_win, e->e_x + e->e_w + 10, e->e_y - 100);

  e->s_win =
      SDL_CreateWindow("Settings", SDL_WINDOWPOS_UNDEFINED,
                       SDL_WINDOWPOS_UNDEFINED, 300, 768, SDL_WINDOW_SHOWN);
...
static void init_textures(Editor *e, AssetMap *map) {
  ...
  e->cur_sheet_index = 0;
  e->total_sheets = load_asset_directory(
      e, "./bin/assets/Rogue/Rogue Adventure World 3.0.0/Tilesets (32x32)");
  if (e->total_sheets > 0) {
    SDL_Renderer *renderers[] = {e->e_render, e->t_render};

    for (int i = 0; i < e->total_sheets; i++) {
      SDL_Texture **textures =
          load_asset_multi_render(renderers, 2, e->d_asset_dir.paths[i]);
...
static void init_settings(Editor *e) {
  ...
  e->settings.is_entity = 0;
  e->settings.frame_rows = 1;
  e->settings.frame_columns = 4;
  e->settings.frame_duration = 0.5f;
