...
AssetChild *find_matching_child(AssetChild *child, char *path) {
  ...
  GINFO("Searching for path: %s", path);

  while (strcmp(temp->path, path) != 0) {
    GINFO("Path at current asset: %s", temp->path);
    temp = temp->next_child;
  }
...
int add_asset(AssetMap *map, SDL_Texture **textures, int render_count,
              char *path) {
  ...
  if (map->count < MAX_ASSET_FILES) {
    ...
    GINFO("Collision at index: %d", index);
...
SDL_Texture *load_asset_to_window(SDL_Renderer *renderer, char *file_path,
...
SDL_Texture **load_asset_multi_render(SDL_Renderer **renderers,
                                      int render_count, char *file_path) {
  SDL_Surface *surface = IMG_Load(file_path);
  if (surface == NULL) {
    GFATAL("Failed to load spritesheet: %s\n", IMG_GetError());
    return NULL;
...
void free_asset(SDL_Texture *tilesheet) { free(tilesheet); }
